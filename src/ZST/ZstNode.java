...
public class ZstNode extends Thread {
	...
	public Map<String, ZstMethod> getMethods() {
		return m_methods;
	}

	protected Map<String, ZstMethod> m_methods;

	public Map<String, ZstPeerlink> getPeers() {
		return m_peers;
	}
	 ...
	 * 
	 * @param nodeId
	 *            Unique name identifying this stage nodev
	 * @param stagePort
	 *            Port to run stage on
	 */
	...
	/**
	 * Create a new node that registers itself to a stage node
	 * 
	 * @param nodeId
	 *            Unique name identifying this node
	 * @param stageAddress
	 *            Address of the stage in the format 'address:port' (eg:
	 *            127.0.0.1:6000)
	 */
	public ZstNode(String nodeId, String stageAddress) {
		//127.0.0.1 breaks so we remap the address to 0.0.0.0
		if(stageAddress.startsWith("127.0.0.1")){
			String port = stageAddress.split(":")[1];
			stageAddress = "0.0.0.0:" + port;
		}
		init(nodeId, stageAddress);
	}

	/**
	 * Internal node bootstrapper
	 * 
	 * @param nodeId
	 *            Unique name identifying this node
	 * @param stageAddress
	...
	private void initNetwork() {
		...
		m_publisherAddress = (String) zmq.ZMQ.zmq_getsockoptx(
				m_publisher.base(), zmq.ZMQ.ZMQ_LAST_ENDPOINT);

		if (m_stageAddress != null) {
			m_reply.bind(address + ":*");
			m_replyAddress = (String) zmq.ZMQ.zmq_getsockoptx(m_reply.base(),
					zmq.ZMQ.ZMQ_LAST_ENDPOINT);

			m_stage = m_ctx.createSocket(ZMQ.REQ);
			m_stage.setLinger(0);
			System.out.println("Attempting to connect to stage at 'tcp://" + m_stageAddress + "'");
			m_stage.connect("tcp://" + m_stageAddress);

			if (m_verbose) {
				System.out.println("Stage located at "
						+ (String) zmq.ZMQ.zmq_getsockoptx(m_stage.base(),
								zmq.ZMQ.ZMQ_LAST_ENDPOINT));
				System.out.println("Node reply on address " + m_replyAddress);
				System.out.println("Node publisher on address "
						+ m_publisherAddress);
			}
			System.out.println("Showtime active!");
		} else {
			m_reply.bind(address + ":" + m_stagePort);
		}
	 ...
	 */
	private void registerInternalMethods() throws NoSuchMethodException,
			SecurityException {
		Method replyRegisterNode_callback = ZstNode.class.getDeclaredMethod(
				"replyRegisterNode", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(REPLY_REGISTER_NODE, new ZstMethod(
				REPLY_REGISTER_NODE, m_nodeId, ZstMethod.RESPONDER, null, this,
				replyRegisterNode_callback));

		Method replyRegisterMethod_callback = ZstNode.class.getDeclaredMethod(
				"replyRegisterMethod", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(REPLY_REGISTER_METHOD, new ZstMethod(
				REPLY_REGISTER_METHOD, m_nodeId, ZstMethod.RESPONDER, null,
				this, replyRegisterMethod_callback));

		Method replyNodePeerlinks_callback = ZstNode.class.getDeclaredMethod(
				"replyNodePeerlinks", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(REPLY_NODE_PEERLINKS, new ZstMethod(
				REPLY_NODE_PEERLINKS, m_nodeId, ZstMethod.RESPONDER, null,
				this, replyNodePeerlinks_callback));

		Method replyMethodList_callback = ZstNode.class.getDeclaredMethod(
				"replyMethodList", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(REPLY_METHOD_LIST, new ZstMethod(
				REPLY_METHOD_LIST, m_nodeId, ZstMethod.RESPONDER, null, this,
				replyMethodList_callback));

		Method replyAllPeerMethods_callback = ZstNode.class.getDeclaredMethod(
				"replyAllPeerMethods", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(REPLY_ALL_PEER_METHODS, new ZstMethod(
				REPLY_ALL_PEER_METHODS, m_nodeId, ZstMethod.RESPONDER, null,
				this, replyAllPeerMethods_callback));

		Method disconnectPeer_callback = ZstNode.class.getDeclaredMethod(
				"disconnectPeer", new Class[] { ZstMethod.class });
		m_internalNodeMethods.put(DISCONNECT_PEER, new ZstMethod(
				DISCONNECT_PEER, m_nodeId, ZstMethod.WRITE, null, this,
	...
	private void receiveMessage(MethodMessage recv) {
		...
		if(recv.data != null){
			if (recv.data.getOutput() != null){
				if (m_verbose){
					System.out.print("' for '" + recv.data.getNode()
							+ "' with value '"
							+ recv.data.getOutput().toString());
					System.out.println("'");
				}
			}
		}
		...
		if (recv.data != null) {
			if (m_peers.containsKey(recv.data.getNode())) {
				...
				if (peer.getMethods().containsKey(recv.method)) {
					ZstMethod peerMethod = m_peers
							.get(recv.data.getNode()).getMethods()
							.get(recv.method);
					if (peerMethod.getLocalCallbackMethod() != null
							&& peerMethod.getLocalCallbackObject() != null) {
						Object callbackObj = peerMethod
								.getLocalCallbackObject();
						try {
							peerMethod.getLocalCallbackMethod().invoke(
									callbackObj, recv.data);
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
	...
	private Object disconnectPeer(ZstMethod methodData) {
		if (m_verbose)
			System.out.println("Peer '" + methodData.getNode()
					+ "' is leaving.");
		if (m_peers.containsKey(methodData.getNode())) {
			try {
				m_subscriber.disconnect(m_peers.get(methodData.getNode())
						.getPublisherAddress());
	...
	public Boolean close() {
		...
		ZstIo.send(m_publisher, DISCONNECT_PEER, new ZstMethod(DISCONNECT_PEER,
				m_nodeId));
	 ...
	 * 
	 * @param Socket
	 *            Socket to register through
	 */
	public Boolean requestRegisterNode(Socket socket) {
		if (m_verbose)
			System.out
					.println("REQ-->: Requesting remote node to register our addresses. Reply:"
							+ m_replyAddress
							+ ", Publisher:"
							+ m_publisherAddress);
		...
		ZstMethod request = new ZstMethod(REPLY_REGISTER_NODE, m_nodeId,
				ZstMethod.RESPONDER, requestArgs);
		...
		if (message.method.equals(OK)) {
			if (m_verbose)
				System.out
						.println("REP<--: Remote node acknowledged our addresses. Reply:"
								+ m_replyAddress
								+ ", Publisher:"
								+ m_publisherAddress);
			return true;
			...
			if (m_verbose)
				System.out.println("REP<--:Remote node returned "
						+ message.method + " instead of " + OK);
		}
		return false;
	}
	...
	protected Object replyRegisterNode(ZstMethod methodData) {
		...
		m_peers.put(
				nodeId,
				new ZstPeerlink(nodeId, methodData.getArgs()
						.get(ZstPeerlink.REPLY_ADDRESS).toString(), methodData
						.getArgs().get(ZstPeerlink.PUBLISHER_ADDRESS)
						.toString()));

		subscribeToNode(m_peers.get(nodeId));
		ZstIo.send(m_reply, OK);
		System.out.println("Registered node '" + nodeId + "'. Reply:"
				+ m_peers.get(nodeId).getReplyAddress() + ", Publisher:"
				+ m_peers.get(nodeId).getPublisherAddress());
		return null;
	}
	 ...
	 *            Method name.
	 * @param accessMode
	 *            Access mode for method (READ, WRITE, RESPONDER).
	 */
	 ...
	 *            Arguments for method.
	 * @param socket
	 *            Socket to send request through.
	 */
	public void requestRegisterMethod(String method, String accessMode,
			Map<String, Object> args, Socket socket) {
	 ...
	 *            Access mode for method (READ, WRITE, RESPONDER).
	 * @param callbackobj
	 *            Object to run callback on.
	 * @param callback
	 *            Callback method.
	 */
	public void requestRegisterMethod(String method, String accessMode,
			Object callbackObject, Method callback) {
		requestRegisterMethod(method, accessMode, null, callbackObject,
				callback, m_stage);
	}
	 ...
	 */
	public void requestRegisterMethod(String method, String accessMode,
			Map<String, Object> args, Object callbackObject, Method callback) {
		requestRegisterMethod(method, accessMode, args, callbackObject,
				callback, m_stage);
	}
	...
	public void requestRegisterMethod(String method, String accessMode,
			...
			Socket socket) {
		if (m_verbose)
			System.out.println("REQ-->: Registering method " + method
					+ " with remote node and args " + args.toString());

		// Register local copy of our method first
		m_methods.put(method, new ZstMethod(method, m_nodeId, accessMode, args,
		...
		if (msg.method.equals(OK))
			if (m_verbose)
				System.out
						.println("REP<--: Remote node acknowledged our method '"
								+ method + "'");
			else if (m_verbose)
				System.out.println("REP<--:Remote node returned " + msg.method
						+ " instead of " + OK);
	}
	...
	protected Object replyRegisterMethod(ZstMethod methodData) {
		if (m_peers.get(methodData.getNode()).getMethods()
				.containsKey(methodData.getName()))
			System.out.println("'" + methodData.getName()
					+ "' already registered on node " + methodData.getNode()
					+ ". Overwriting");

		m_peers.get(methodData.getNode()).getMethods().put(methodData.getName(), methodData.clone());

		ZstIo.send(m_reply, OK);
		System.out
				.println("Registered method '" + methodData.getName()
						+ "'. Origin:" + methodData.getNode() + ", AccessMode:"
						+ methodData.getAccessMode() + ", Args:"
						+ methodData.getArgs());
		return null;
	}
	...
	protected Object replyNodePeerlinks(ZstMethod methodData) {
		...
		ZstMethod request = new ZstMethod(REPLY_NODE_PEERLINKS, m_nodeId, "",
				null);
	...
	/**
	 * Request a list of all available methods provided by all connected peers
	 * on the remote node</summary>
	 * 
	 * @param socket
	 * @return Map of methods
	 */
	public Map<String, ZstMethod> requestAllPeerMethods(Socket socket) {
		ZstIo.send(socket, REPLY_METHOD_LIST);
		String jsonStr = ZstIo.recv(socket).data.getOutput().toString();
		JsonObject peerMethodList = (JsonObject) ZstIo.jsonParser
				.parse(jsonStr);
		return ZstMethod.buildLocalMethods(peerMethodList);
	}

	/**
	 * Reply with a list of all available methods provided by all connected
	 * peers on the remote node
	 * 
	 * @param methodData
	 * @return null
	 */
	public Object replyAllPeerMethods(ZstMethod methodData) {
		Map<String, Object> methodDict = new HashMap<String, Object>();
		for (Entry<String, ZstPeerlink> peer : m_peers.entrySet()) {
			for (Entry<String, ZstMethod> method : peer.getValue().getMethods()
					.entrySet())
				methodDict.put(method.getKey(), method.getValue().asMap());
		}
		ZstMethod request = new ZstMethod(REPLY_NODE_PEERLINKS, m_nodeId, "",
	...
	public ZstMethod updateRemoteMethod(ZstMethod method,
		...
		if (m_peers.containsKey(method.getNode())) {
			if (ZstMethod.compareArgLists(m_peers.get(method.getNode())
					.getMethods().get(method.getName()).getArgs(), args)) {
				ZstMethod methodRequest = new ZstMethod(method.getName(),
						method.getNode(), method.getAccessMode(), args);
	 ...
	 * 
	 * @param method
	 *            Remote method to subscribe to
	 * @param callbackObject
	 *            Object to run callback on
	 * @param callbackMethod
	 *            Callback to run
	 */
	public void subscribeToMethod(ZstMethod method, Object callbackObject,
			Method callbackMethod) {
		if (m_peers.containsKey(method.getNode())) {
			ZstMethod remoteMethod = m_peers.get(method.getNode()).getMethod(method.getName());
